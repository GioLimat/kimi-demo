...
fn fmt_f32_fixed10(mut f: f32) -> String {
    let factor = 10_f32.powi(7);
    f = (f * factor).round() / factor;
    let s = format!("{:.7}", f);
    if let Some(dot) = s.find('.') {
        let dec_len = s.len() - dot - 1;
        let zeros = "0".repeat(10 - dec_len);
        format!("{}{}", s, zeros)
    } else {
        format!("{}.{}", s, "0".repeat(10))
    }
}

fn format_array(vm: &VM, id: u64) -> String {
    let (elem_type, _, elements) = vm.heap_manager.get(id).unwrap().as_array().unwrap();
    let items: Vec<String> = elements.iter().map(|&raw| format_value(vm, *elem_type, raw)).collect();
    format!("[{}]", items.join(", "))
}
...
fn format_value(vm: &VM, elem_type: u8, raw: u64) -> String {
    match elem_type {
        ...
        0x03 => fmt_f32_fixed10(f32::from_bits(raw as u32)),
        0x04 => format!("{:.10}", f64::from_bits(raw)),
        0x05 => if raw == 0 { "false".into() } else { "true".into() },
        0x09 => std::char::from_u32(raw as u32).map(|c| c.to_string()).unwrap_or_else(|| raw.to_string()),
        0x0A => if let HeapValue::String(s) = vm.heap_manager.get(raw).unwrap() { format!("\"{}\"", s) } else { raw.to_string() },
...
pub fn op_println(vm: &mut VM) {
    ...
    match op_type {
        ...
        0x09 => {
            if let Some(c) = std::char::from_u32(value as u32) { println!("{}", c) } else { println!("{}", value) }
        }
        0x0A => if let HeapValue::String(s) = vm.heap_manager.get(value).unwrap() { println!("\"{}\"", s) } else { println!("{}", value) },
