...
static inline void mul_f64(OrionVM& vm) {
    RawValue b = vm.popValue(), a = vm.popValue();
    double da, db;
...
static inline void mul_i8(OrionVM& vm) {
    RawValue b = vm.popValue(), a = vm.popValue();
    vm.pushValue(static_cast<RawValue>(static_cast<int8_t>(a) * static_cast<int8_t>(b)));
}

static inline void mul_i16(OrionVM& vm) {
    RawValue b = vm.popValue(), a = vm.popValue();
    vm.pushValue(static_cast<RawValue>(static_cast<int16_t>(a) * static_cast<int16_t>(b)));
}

static inline void mul_char(OrionVM& vm) {
    RawValue b = vm.popValue(), a = vm.popValue();
    vm.pushValue(static_cast<uint32_t>(a) * static_cast<uint32_t>(b));
}

static constexpr MulFn mulTable[256] = {
    nullptr, mul_i32, mul_i64, mul_f32, mul_f64, nullptr, nullptr,
    mul_i8, mul_i16, mul_char
};

#endif //MUL_HANDLERS_H
